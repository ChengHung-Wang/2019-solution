        ...
        <div class="row">
            <div class="col-12 col-lg-4 mb-3">
                ...
                <select class="form-control @error('type') is-invalid @enderror" id="selectType" name="type">
                    ...
                    <option value="workshop" {{ old('type', $session->type) === 'workshop' ? 'selected' : ''  }}>
                        Workshop
                    </option>
                </select>
                @error('type')
                ...
                <select class="form-control @error('room_id') is-invalid @enderror" id="selectRoom" name="room_id">
                    @foreach($event->channels as $channel)
                        @foreach($channel->rooms as $room)
                            <option
                                value="{{$room->id}}" {{$room->id === old('room_id', $session->room_id) ? 'selected' : ''}}>{{$room->name}}
                                / {{$channel->name}}</option>
                        @endforeach
                    @endforeach
                </select>
        ...
        <div class="row">
            <div class="col-12 col-lg-6 mb-3">
                ...
                <input type="text"
                       ...
                       value="{{ old('start', $session->start->format('Y-m-d H:i')) }}">
                @error('start')
                <div class="invalid-feedback">
                    {{$message}}
                </div>
            ...
            <div class="col-12 col-lg-6 mb-3">
                ...
                <input type="text"
                       ...
                       name="end"
                       placeholder="yyyy-mm-dd HH:MM"
                       value="{{ old('end', $session->end->format('Y-m-d H:i')) }}">
                @error('end')
                <div class="invalid-feedback">
                    {{$message}}
                </div>
